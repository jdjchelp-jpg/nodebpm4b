...
async function generateChapterAudio(chapterText, outputDir, config, onProgress = null, jobId = null, chapterIndex = 0) {
    ...
    while (retryCount < MAX_RETRIES) {
        ...
        // Check for severely truncated chunks (indicates model limitation)
        ...
        for (let i = 0; i < audioBuffers.length; i++) {
            ...
            // If more than 80% truncated, re-chunk with smaller size
            if (actualDuration < expectedDuration * 0.2) {
                console.warn(`[TTS Engine] Chunk ${i+1} severely truncated! Expected ~${expectedDuration.toFixed(1)}s, got ${actualDuration.toFixed(1)}s (${allChunks[i].text.length} chars)`);
                hasTruncation = true;
            }
        }
