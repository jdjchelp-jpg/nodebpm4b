...
const MAX_CHUNK_LENGTH = 500; // Reduced from 1000 for better stability and complete coverage
const MIN_CHUNK_LENGTH = 50; // Increased from 20 for better quality
const CHUNK_OVERLAP = 20; // Overlap chunks to prevent cutting words
 ...
 * IMPROVED: Ensures no text is lost by using overlap and better boundary detection.
 * @param {string} text
 * @param {number} maxLength
 * @returns {string[]}
 */
function splitTextIntoChunks(text, maxLength = MAX_CHUNK_LENGTH) {
    ...
    while (position < totalLength) {
        ...
        // Calculate the end of this chunk (with overlap for safety)
        let chunkEnd = position + maxLength;
        ...
        const searchRange = text.substring(position, chunkEnd);
        let splitIndex = -1;

        // Priority 1: Sentence endings (. ! ?) followed by space or newline
        const sentenceEnds = ['. ', '! ', '? ', '.\n', '!\n', '?\n', '.\r', '!\r', '?\r'];
        ...
        // Priority 4: Word boundaries (spaces, tabs)
        if (splitIndex <= 0) {
            const space = searchRange.lastIndexOf(' ');
            if (space > 0) splitIndex = space + 1;
            else {
                const tab = searchRange.lastIndexOf('\t');
                if (tab > 0) splitIndex = tab + 1;
            }
        }

        // Priority 5: Punctuation boundaries (commas, semicolons, colons)
        if (splitIndex <= 0) {
            const punctuation = searchRange.lastIndexOf(',');
            if (punctuation > 0) splitIndex = punctuation + 1;
            else {
                const semicolon = searchRange.lastIndexOf(';');
                if (semicolon > 0) splitIndex = semicolon + 1;
                else {
                    const colon = searchRange.lastIndexOf(':');
                    if (colon > 0) splitIndex = colon + 1;
                }
            }
        }

        // Priority 6: Any non-letter/digit boundary
        if (splitIndex <= 0) {
        ...
        // Absolute fallback: split at maxLength (should never happen with reasonable text)
        if (splitIndex <= 0) {
            splitIndex = maxLength;
            console.warn('[TTS Chunking] Forced split at maxLength - no suitable boundary found');
        }

        // Extract the chunk
        const chunk = text.substring(position, position + splitIndex).trim();
        if (chunk.length > 0) {
            chunks.push(chunk);
        }

        // Move position forward (with small overlap to prevent cutting words)
        position = position + splitIndex;
        
        // Add overlap if we're not at the end
        if (position < totalLength && CHUNK_OVERLAP > 0) {
            position = Math.max(position, position - CHUNK_OVERLAP);
        }
    }

    // Validate that we didn't lose any text
    const totalChunkedLength = chunks.join('').length;
    if (totalChunkedLength < totalLength * 0.95) {
        console.warn(`[TTS Chunking] Possible text loss: original ${totalLength} chars, chunked ${totalChunkedLength} chars (${(totalChunkedLength/totalLength*100).toFixed(1)}%)`);
    }
 ...
 * IMPROVED: Better truncation detection, validation, and faster processing.
...
async function generateChapterAudio(chapterText, outputDir, config, onProgress = null, jobId = null, chapterIndex = 0) {
    ...
    // Track total characters for validation
    const totalInputChars = chapterText.length;
    ...
    while (retryCount < MAX_RETRIES) {
        ...
        const totalChunkedChars = allChunks.reduce((sum, chunk) => sum + chunk.text.length, 0);
        const coveragePercent = (totalChunkedChars / totalInputChars) * 100;
        
        console.log(`[TTS Engine] Chapter ${chapterId}: Split into ${allChunks.length} chunks (max ${chunkSize} chars each)`);
        console.log(`[TTS Engine] Text coverage: ${totalChunkedChars}/${totalInputChars} chars (${coveragePercent.toFixed(1)}%)`);

        if (coveragePercent < 95) {
            console.warn(`[TTS Engine] WARNING: Low text coverage (${coveragePercent.toFixed(1)}%) - some text may be lost`);
        }
        ...
        console.log(`[TTS Engine] Chapter ${chapterId}: Processed ${allChunks.length} chunks in ${processingTime.toFixed(2)}s (${(allChunks.length/processingTime).toFixed(1)} chunks/s)`);

        if (jobId && isJobCancelled(jobId)) throw new Error('CANCELLED');

        // IMPROVED truncation detection with better heuristics
        const expectedCharsPerSecond = 15; // Average speaking rate
        ...
        for (let i = 0; i < audioBuffers.length; i++) {
            ...
            // More lenient threshold: only flag if severely truncated (< 50% of expected)
            // This reduces false positives while still catching real issues
            if (actualDuration < expectedDuration * 0.5) {
                console.warn(`[TTS Engine] Chunk ${i+1} severely truncated! Expected ~${expectedDuration.toFixed(1)}s, got ${actualDuration.toFixed(1)}s (${allChunks[i].text.length} chars, ${(actualDuration/expectedDuration*100).toFixed(0)}% of expected)`);
                hasTruncation = true;
                truncatedChunks++;
            }
        }
        ...
        if (hasTruncation) {
            console.warn(`[TTS Engine] ${truncatedChunks}/${allChunks.length} chunks truncated (${(truncatedChunks/allChunks.length*100).toFixed(1)}%)`);
        }
        ...
        if (!hasTruncation) {
            ...
            // Validate final output duration matches expected
            const expectedTotalDuration = totalChunkedChars / expectedCharsPerSecond;
            const durationMatch = (duration / expectedTotalDuration) * 100;
            
            console.log(`[TTS Engine] Chapter ${chapterId}: Final duration ${duration.toFixed(2)}s (expected ~${expectedTotalDuration.toFixed(2)}s, ${durationMatch.toFixed(1)}% match)`);
            
            if (durationMatch < 70) {
                console.warn(`[TTS Engine] WARNING: Final duration significantly shorter than expected (${durationMatch.toFixed(1)}% match). Some text may have been skipped.`);
            }
        ...
        retryCount++;
        chunkSize = Math.max(MIN_CHUNK_LENGTH, Math.floor(chunkSize / 2));
        ...
        if (chunkSize <= MIN_CHUNK_SIZE) {
            throw new Error(`[TTS Engine] Model persistently truncates even at ${MIN_CHUNK_LENGTH} chars. This may indicate a model or memory issue. Try a different voice or restart the server.`);
        }
    }
