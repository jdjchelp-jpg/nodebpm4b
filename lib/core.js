...
async function convertToM4b(inputPath, outputPath, chapters = null, options = {}) {
  ...
  return new Promise(async (resolve, reject) => {
    let chapterFile = null;
    const args = ['-hide_banner', '-loglevel', 'verbose', '-i', inputPath];
...
async function folderToM4b(folderPath, outputPath, options = {}) {
  ...
  report(5, `Found ${mediaFiles.length} media files. Processing...`);
  ...
  try {
    ...
    for (let i = 0; i < mediaFiles.length; i++) {
      ...
      report(
        5 + Math.round((i / mediaFiles.length) * 60),
        `Normalizing ${i + 1}/${mediaFiles.length}: ${mf.name}`
      );

      const normalizedPath = path.join(tempDir, `part_${String(i + 1).padStart(4, '0')}.wav`);
      normalizedFiles.push(normalizedPath);

      // Normalize each file to consistent WAV format for concatenation
      await new Promise((resolve, reject) => {
        const args = [
          '-hide_banner', '-loglevel', 'error',
          '-i', mf.fullPath,
          '-vn', // Drop video if present, keep only audio
          '-c:a', 'pcm_s16le',
          '-ar', '44100',
          '-ac', '2',
          '-y', normalizedPath
        ...
        proc.on('close', (code) => {
          if (code === 0) resolve();
          else reject(new Error(`FFmpeg normalize exited with code ${code}`));
        });
    ...
    // Concatenate all normalized WAVs into a single WAV
