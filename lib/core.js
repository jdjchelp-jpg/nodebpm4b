 ...
 * @param {string} encoderName - Name of the encoder (e.g., 'libfdk_aac')
 * @returns {Promise<boolean>}
 */
  ...
  return new Promise((resolve) => {
    exec(`"${ffmpegPath}" -encoders`, (error, stdout) => {
      if (error) {
        ...
        resolve(stdout.includes(encoderName));
      }
    });
  });
}
...
/**
 * Get the best available AAC encoder
 * @returns {Promise<string>} 'libfdk_aac' or 'aac'
 */
async function getBestAacEncoder() {
  const hasFdk = await checkEncoder('libfdk_aac');
  return hasFdk ? 'libfdk_aac' : 'aac';
}
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      if (audioQuality === 'copy') {
        ...
        // Prioritize libfdk_aac for superior quality
        const aacEncoder = await getBestAacEncoder();
        args.push('-c:a', aacEncoder);
        
        // Add specific flags for libfdk_aac if used
        if (aacEncoder === 'libfdk_aac') {
          args.push('-vbr', '4'); // Use high-quality VBR for FDK
        } else {
          args.push('-b:a', validQuality);
        }
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      if (audioQuality === 'copy') {
        ...
        // Prioritize libfdk_aac
        const aacEncoder = await getBestAacEncoder();
        args.push('-c:a', aacEncoder);
        
        if (aacEncoder === 'libfdk_aac') {
          args.push('-vbr', '3'); // Good balance for general audio
        } else {
          args.push('-b:a', validQuality);
        }
