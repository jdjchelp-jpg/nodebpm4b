...
function createServer(options = {}) {
    ...
    const cloneUpload = upload.fields([
        { name: 'reference_audio', maxCount: 1 },
        { name: 'source_audio', maxCount: 1 }
    ]);

    app.post('/api/clone-voice', cloneUpload, async (req, res) => {
        try {
            const refFile = req.files['reference_audio'] ? req.files['reference_audio'][0] : null;
            const srcFile = req.files['source_audio'] ? req.files['source_audio'][0] : null;
            const mode = req.body.mode || 'tts';
            const text = req.body.text || '';
            const lang = req.body.lang || 'en';
            const jobId = req.body.jobId;

            if (!refFile) {
                return res.status(400).json({ error: 'Reference audio required' });
            }
            ...
            let cmd = `python kokoclone/cli.py --mode ${mode} --ref "${refFile.path}" --out "${outputPath}"`;
            if (mode === 'tts') {
                if (!text) return res.status(400).json({ error: 'Text required for TTS mode' });
                cmd += ` --text "${text.replace(/"/g, '\\"')}" --lang ${lang}`;
            } else {
                if (!srcFile) return res.status(400).json({ error: 'Source audio required for conversion mode' });
                cmd += ` --source "${srcFile.path}"`;
            }

            const client = progressClients.get(jobId);
            if (client) {
                client.write(`data: ${JSON.stringify({ percent: 10, message: 'Waking up Neural Core...' })}\n\n`);
            }
            ...
            exec(cmd, (error, stdout, stderr) => {
                if (error) {
                    ...
                    if (client) {
                        client.write(`data: ${JSON.stringify({ percent: 0, message: 'Error: Voice Cloning failed. Ensure Python and dependencies are installed.' })}\n\n`);
                    }
                    return;
                }
            ...
            res.json({ success: true, message: 'Cloning started', jobId });

        } catch (error) {
            console.error('Fatal error in clone-voice:', error);
            res.status(500).json({ error: error.message });
        }
    });
