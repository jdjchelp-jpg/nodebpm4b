...
function createServer(options = {}) {
    ...
    app.get('/api/engine-info', async (req, res) => {
        try {
            const audiblezInstalled = await isAudiblezInstalled();
            const audiblezVersion = audiblezInstalled ? await getAudiblezVersion() : 'not installed';
            
            res.json({
                kokoro: { status: 'ready', type: 'local-pool' },
                audiblez: { 
                    status: audiblezInstalled ? 'ready' : 'missing',
                    version: audiblezVersion
                }
            });
        } catch (error) {
            res.status(500).json({ error: error.message });
    ...
    app.get('/api/voices', async (req, res) => {
        try {
            ...
            // Try to add Audiblez voices if installed
            try {
                if (await isAudiblezInstalled()) {
                    const audiblezVoices = await getAudiblezVoices();
                    audiblezVoices.forEach(vId => {
                ...
                console.warn('[Server] Failed to fetch Audiblez voices:', err.message);
            }
    ...
    // ── EPUB to Audiobook Endpoints (NEW - Audiblez Integration) ──
        ...
        try {
            ...
            const result = await epubToAudiobook(
                ...
                outputPath,
                {
                    ...
                    engine: engine || 'audiblez', // 'bpm4b' or 'audiblez' (default: audiblez)
    ...
    // ── Document to EPUB Endpoints (NEW - For Audiblez Compatibility) ──
        ...
        try {
            ...
            try {
                ...
                const runGeneration = async () => {
                    ...
                    try {
                        // If Audiblez is selected, ensure we have a clean EPUB
                        const ext = path.extname(docFile.originalname).toLowerCase();
                        if (engine === 'audiblez') {
                            if (ext !== '.epub') {
                                onProgress(5, 'Converting document to EPUB for Audiblez...');
                                ...
                                onProgress(5, 'Healing EPUB structure for maximum Audiblez compatibility...');
                        ...
                        if (engine === 'audiblez') {
                            await epubToAudiobook(currentDocPath, outputPath, {
                                ...
                                jobId,
                                engine: 'audiblez',
                                customPath: req.body.audiblezPath || null
                            });
                        } else {
