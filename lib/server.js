...
const { generateChapterAudio, cancelJob, AVAILABLE_VOICES, AVAILABLE_MODELS } = require('./tts-engine');
const { generateMetadata, analyzeNarration, getModels } = require('./ai-engine');
...
const documentUpload = multer({
    ...
    fileFilter: (req, file, cb) => {
        ...
        const validExts = ['.pdf', '.docx', '.txt', '.epub', '.md'];
...
function createServer(options = {}) {
    ...
    app.post('/api/ai/metadata', async (req, res) => {
        try {
            ...
            const metadata = await generateMetadata(title, author, apiKey, { model });
            res.json(metadata);
        } catch (error) {
            res.status(500).json({ error: error.message });
        }
    ...
    app.post('/api/ai/narration-plan', async (req, res) => {
        try {
            ...
            const plan = await analyzeNarration(text, apiKey, { model });
            res.json(plan);
        } catch (error) {
            res.status(500).json({ error: error.message });
        }
