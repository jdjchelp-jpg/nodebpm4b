...
async function buildAudiobook(inputPath, outputPath, options = {}) {
    ...
    try {
        ...
        const chapterAudios = await generateAllChapterAudio(
            ...
            (chapterIdx, totalChapters, detail) => {
                if (onProgress) {
                    ...
                    const subPercent = 15 + Math.round(((chapterIdx) / totalChapters) * 70);
        ...
        if (onProgress) onProgress(90, 'Combining chapter audio...');
        ...
        if (onProgress) onProgress(95, 'Creating final M4B with embedded chapters...');
        await convertMp3ToM4b(combinedWav, outputPath, chapterMetadata, {
            audioQuality,
            onProgress: (percent, msg) => {
                // percent here is 0-100 for just this stage
                const finalPercent = 95 + Math.round((percent / 100) * 5);
