 ...
 * Integrates EPUB parsing with BPM4B's optimized TTS engine or Audiblez
 * Inspired by Audiblez but using BPM4B's faster TTS implementation
 * Also supports using Audiblez as a subprocess for audiobook generation
 */
...
const { epubToAudiobookWithAudiblez, ensureAudiblezInstalled } = require('./audiblez-integration');
...
async function epubToAudiobook(epubPath, outputPath, options = {}) {
    const {
        ...
        onProgress = null,
        jobId = null,
        engine = 'audiblez' // 'bpm4b' or 'audiblez' (default: audiblez)
    } = options;

    console.log(`[EPUB to Audiobook] Starting conversion of "${epubPath}"`);
    console.log(`[EPUB to Audiobook] Using engine: ${engine}`);

    // If Audiblez is selected, use it
    if (engine === 'audiblez') {
        console.log(`[EPUB to Audiobook] Using Audiblez subprocess`);
        
        // Ensure Audiblez is installed (auto-install if not)
        if (onProgress) onProgress('Checking Audiblez installation...', 0, 100);
        const audiblezInstalled = await ensureAudiblezInstalled();
        
        if (!audiblezInstalled) {
            throw new Error('Failed to install Audiblez automatically. Please install it manually with: pip install audiblez');
        }
        ...
        // Run Audiblez
        if (onProgress) onProgress('Running Audiblez...', 10, 100);
        
        const result = await epubToAudiobookWithAudiblez(epubPath, outputPath, {
            voice: targetVoice,
            speed: speed,
            ...
            onProgress: (progress) => {
                if (onProgress) {
                    onProgress(`Audiblez progress: ${progress}%`, 10 + (progress * 0.9), 100);
                }
            }
        });
